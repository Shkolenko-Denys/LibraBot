...
class RegistrationForm(FlaskForm):
    ...
    def validate_nickname(self, filed):
        """Method to check if the nickname is already registered."""
        if User.query.filter(db.func.lower(User.nickname) ==
                             db.func.lower(filed.data)).first():
            raise ValidationError(u'The Nickname has already been registered')
